...
while s.check() == sat:
    ...
    if true_for_all is None:
        true_for_all = compare.copy()
    else:
        for i in range(len(true_for_all)):
            if true_for_all[i][0] != compare[i][0]:
                true_for_all[i] = (None, 0)
...
if true_for_all.count((None, 0)) != len(true_for_all):
    for v, pos in true_for_all:
        if v is None:
            continue
        tmp_board[pos[0]][pos[1]] = colored(v, 'green')
